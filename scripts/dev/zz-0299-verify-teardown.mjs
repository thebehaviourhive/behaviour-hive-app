...
const { data: strayPassports } = await admin.from("passports").select("id").eq("child_name", "ZZ 0299 Child");
...
const targets = usersList.users.filter((u) => u.email && u.email.startsWith("zz0299."));
...
const { data: instAfter } = await admin.from("institutions").select("id").ilike("institution_code", "ZZ0299%");
const { data: passportsAfter } = await admin.from("passports").select("id").eq("child_name", "ZZ 0299 Child");
const { data: usersAfter } = await admin.auth.admin.listUsers({ perPage: 1000 });
const remainingUsers = usersAfter.users.filter((u) => u.email && u.email.startsWith("zz0299."));
