...
// Bulk EOD update -- the single-child wizard extracted out of
// /teacher/eod/[passportId]/page.tsx (untouched in behaviour) so both
// that page and the new bulk flow (/teacher/eod/bulk) share one
// implementation instead of two copies drifting apart. The single-
// child page renders this with onMarkAbsent omitted -- "Absent" is
// scoped to the bulk flow only (see that page's own header comment for
// why), everything else is identical, keyed by passportId so React
// remounts fresh state -- including this component's own clinical-
// content/ask-cap hooks -- for every child in the walk.
...
interface EodWizardForChildProps {
  passportId: string;
  teacherId: string;
  childName: string;
  onDone: () => void;
  // Bulk flow only -- see this file's own header comment.
  progressLabel?: string;
  onMarkAbsent?: () => Promise<void>;
}

export function EodWizardForChild({
  passportId,
  teacherId,
  childName,
  onDone,
  ...
  const { isEligible: underAskCap, recordPrompt } = useStrategyFeedbackCap(passportId, teacherId);
  ...
  useEffect(() => {
    if (!teacherId || !passportId) return;
    let isMounted = true;
  ...
  function handleRateStrategy(itemId: string, rating: StrategyFeedbackRating) {
    ...
    rateStrategyContent({ passportId, strategyContentId: itemId, rating, raterId: teacherId });
  ...
  async function handleMarkAbsent() {
    ...
    try {
      ...
      setAbsentError(err instanceof Error ? err.message : "Couldn't record this.");
      setIsMarkingAbsent(false);
    }
    // On success the caller's own onMarkAbsent is expected to advance
    // (it owns "what happens next" -- move to the next child, or
    // finish the walk). No local success flash here, deliberately --
    // marking absent is a skip, not a completed account, and it should
    // read as fast as it is.
  }
      ...
      <div className="relative flex-1 overflow-hidden">
        ...
        >
          <StepPanel>
            ...
            {/* Bulk flow only -- a fast skip for a child who isn't in
                today, right at the first decision point rather than a
                separate screen ahead of it. Absent from the single-
                child page entirely (onMarkAbsent undefined there). */}
            {onMarkAbsent && (
              <div className="mt-4 border-t border-black/5 pt-4">
                <button
                  ...
                  className="w-full rounded-2xl border-2 border-black/10 bg-white py-3 text-sm font-semibold text-black/60 transition-colors disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {isMarkingAbsent ? "Recording…" : `${childName} isn't in today`}
                </button>
                {absentError && (
