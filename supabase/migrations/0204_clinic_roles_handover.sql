-- PRD 5 Stage 2, enumeration #5 of Stage 1's own audit: hand_over_
-- principal()'s p_staying_role validation, currently a flat
-- ('class_teacher', 'sna') check -- meaningless at a clinic, which has
-- neither role. Same "does the answer differ between school and
-- clinic" shape as the self-link policy (0203): the valid staying-role
-- SET has to be chosen from the institution's own type, not unioned
-- into one flat wider list that would let a school's outgoing principal
-- "become" a clinical_lead.
--
-- The clinic set is three-wide, not two -- clinician, clinical_lead,
-- clinic_admin -- wider than the school's own two, because a departing
-- clinical director going back to clinical practice is a real case the
-- school parallel never had to consider (school clinicians were never
-- institution_staff before this stage; a clinic's practitioners always
-- are, per 0203).
...
-- The table-level CHECK constraint can't do the cross-table lookup the
-- RPC above does (which institution, which type) -- it can only ever
-- enforce "one of these five values total" (the union across both
-- types). The RPC above is what enforces the TYPE-correct subset; this
-- constraint is the cheap universal invariant underneath it, same
-- division of labour this schema already uses elsewhere.
alter table public.principal_handovers
  drop constraint if exists principal_handovers_staying_role_check;
alter table public.principal_handovers
  add constraint principal_handovers_staying_role_check
  check (staying_role is null or staying_role in (
    'class_teacher', 'sna', 'clinician', 'clinical_lead', 'clinic_admin'
  ));
