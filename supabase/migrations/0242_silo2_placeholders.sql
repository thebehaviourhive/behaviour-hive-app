...
--
-- PRD 7 -- the Silo 2 placeholders. Five plan types -- crisis
-- management, sensory diet, AAC/communication, care plans, statutory
-- (IEP and equivalents) -- one generic table (clinical_plans), five
-- clinical_artefact_types rows at silo = 'framework', inheriting Stage
-- 3's domain-tag machinery and the attachments bridge (0232) wholesale.
-- Not "coming soon" cards: a clinician can create one today (name,
-- date, a free-text body, an optional attachment) and it appears on
-- the child's record.
--
-- ***************************************************************************
-- WHY THESE DO NOT LOCK -- SEE THE TABLE'S OWN COMMENT BELOW TOO, SO
-- THIS SURVIVES SCHEMA INTROSPECTION, NOT JUST THIS FILE.
--
-- The attachment is the authoritative artefact. The app record is
-- metadata about a signed document that lives as a file, and locking a
-- pointer to an already-immutable file is theatre. WHEN any of these
-- five graduates to a built-in-full structured form (own fields, own
-- editor, no longer just a name+attachment pointer), it inherits BSP's
-- own lifecycle -- draft, locked on sign, revision as a new row, never
-- an edit. Without that note on record, whoever builds a structured
-- crisis plan later on this already-existing unlocked table has no
-- reason to notice the lock needs adding at that point, not before.
-- ***************************************************************************
--
-- ***************************************************************************
-- REQUIREMENT 2, TRACED THROUGH RATHER THAN ASSERTED: A SCHOOL CAN SEE
-- THAT A PLAN EXISTS AND READ ITS BODY, AND CANNOT FETCH THE SIGNED
-- FILE. This is the one place two rules meet and could contradict --
-- Silo 3's "shareable by default" against Stage 2's "the attachment
-- stays clinic-only, always" -- so it is proven live below, not just
-- asserted here. The reason it CANNOT contradict, structurally:
-- attachments' own authorization helpers (_caller_owns_artefact,
-- _caller_can_modify_artefact_attachments) dispatch on artefact_type
-- and, for the new 'clinical_plan' arm, check ONLY clinical_plans.
-- clinician_id = auth.uid() -- the exact same narrow shape the
-- 'assessment' arm already has (author-only, no director, no domain-
-- colleague, and certainly no school branch -- a known, already-
-- flagged asymmetry from 0232/0233, not something this migration
-- widens). No column on attachments inherits clinical_plans' own
-- school_visibility_override, because no such column exists at all --
-- there is nothing FOR a school branch to key off of even if someone
-- tried to add one later without also adding the column. The body
-- (clinical_plans.name/plan_date/body) and the file (attachments/
-- storage.objects) are two GENUINELY SEPARATE authorization surfaces,
-- and only the first one's policies ever mention school visibility.
-- ***************************************************************************
...
-- communication both map cleanly; statutory_plan -> statutory_education
-- uses the domain enum value that appears to have been named with
-- exactly this in mind. crisis_plan and care_plan are left '{}'
...
alter table public.clinical_artefact_types
  add constraint clinical_artefact_types_artefact_type_check
    check (artefact_type in (
      ...
      'crisis_plan', 'sensory_diet_plan', 'aac_plan', 'care_plan', 'statutory_plan'
    ));
...
insert into public.clinical_artefact_types (artefact_type, silo, default_domain_tags, default_school_visibility) values
  ...
  ('statutory_plan', 'framework', array['statutory_education']::public.clinical_domain[], 'shareable')
on conflict (artefact_type) do nothing;

-- ===========================================================================
-- 2. clinical_plans -- the one generic table backing all five types,
...
create table public.clinical_plans (
  ...
  plan_type text not null references public.clinical_artefact_types (artefact_type)
    check (plan_type in ('crisis_plan', 'sensory_diet_plan', 'aac_plan', 'care_plan', 'statutory_plan')),
  name text not null,
...
comment on table public.clinical_plans is
  'Silo 2 placeholders: crisis management, sensory diet, AAC/communication, care plans, statutory (IEP and equivalents). Deliberately does NOT lock -- the attachment (when one exists) is the authoritative, already-immutable artefact; this row is metadata about it (a name, a date, a free-text summary), and locking a pointer to an immutable file is theatre. WHEN any one of these five graduates to a built-in-full structured form (own fields, own editor, no longer just a name+attachment pointer), it inherits bsp''s own lifecycle: draft, locked on sign, revision as a new row, never an edit. This comment is why that lock does not exist YET on this table -- not an oversight to fix quietly later.';
...
-- value -- the helper below folds the per-instance override and the
-- type-level default into one check, so this policy (and the
-- principal branch, and the RPC in part 3) never re-derive that logic
-- three separate ways.
...
-- ===========================================================================
-- 3. The school-visibility helper AND the read RPC. The helper folds
-- override-then-type-default into one place; the RPC exists (rather
-- than a raw client select) purely to join author_name from auth.users,
-- the same reason get_passport_clinical_content() is an RPC and not a
-- bare select -- neither a teacher, a principal, nor a parent can read
-- another user's own auth.users row directly.
-- ===========================================================================
...
alter table public.attachments
  drop constraint attachments_artefact_type_check;
alter table public.attachments
