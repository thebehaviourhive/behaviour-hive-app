...
**Owed to this stage's build prompt, VERIFY section — do not omit when Stage 1b is written up:** Stage 1 shipped four structural-only adversarial checks that could not be live-fired because no principal could be deactivated by any real path — `deactivate_institution_staff()`'s last-principal guard, `can_countersign_incident()`'s principal branch, `incident_locations` add/edit principal branches, and `mark_parent_called()`'s principal branch (see CLAUDE.md, "Deferred work"). Stage 1b (join approval) added a fifth of the same kind: `approve_staff_join()`/`reject_staff_join()`'s caller check and `derive_staff_join_approval()`'s EXISTS clause both correctly exclude a pending-or-rejected principal from counting as active, but no such row can exist to test against — a second principal-role insert at an institution that already has one fails at `institution_staff_one_principal_per_institution` outright, confirmed empirically, before it can persist in any state. Handover with outcome 'leaving' makes a genuinely deactivated principal reachable for the first time, and with it, a genuine window where a second principal-role join could exist as pending or rejected rather than just blocked. Stage 1b's own verification must convert all five to live-fired checks against a real handed-over, deactivated principal, not leave them structural now that the state they protect against is real.

**Also owed to this stage's build prompt, in scope, not deferred:** the same finding has a design half, not just a test-coverage half. Today, a second principal-role join at an institution that already has one surfaces a raw `duplicate key value violates unique constraint` to the caller — no designed message, no pending state. The person hitting this is very likely a genuine successor arriving before handover exists to receive them. This stage's Step 3 must replace that raw error with a designed message pointing them at handover (ask the current principal to hand over / contact support) — not a friendlier pending state, since a second principal was never meant to queue.
