...
**Four of these six convert with this stage, not five as an earlier pass through this section claimed** — `can_countersign_incident()`'s principal branch, `incident_locations` add/edit, `mark_parent_called()`'s principal branch, and the `/principal/dashboard` overlay browser check. Each just needs SOME real deactivated principal to exist, which `hand_over_principal()`'s 'leaving' outcome now produces (structurally identical in shape to what `deactivate_institution_staff()` would have produced, had it ever been able to target a principal).
